...
public class OriginSlimeOverlayFeatureRenderer<T extends LivingEntity> extends FeatureRenderer<T, OriginSlimeModel<T>> {
    private final EntityModel<T> model = new OriginSlimeModel(0);

    public OriginSlimeOverlayFeatureRenderer(FeatureRendererContext<T, OriginSlimeModel<T>> featureRendererContext) {
        super(featureRendererContext);
    }

    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        if (!livingEntity.isInvisible()) {
            ((OriginSlimeModel)this.getContextModel()).copyStateTo(this.model);
